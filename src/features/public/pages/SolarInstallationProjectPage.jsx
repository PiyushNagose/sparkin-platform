...
function getVendorName(project) {
  return project?.vendorEmail
    ? project.vendorEmail.split("@")[0]
    : "Assigned Vendor";
}
...
export default function SolarInstallationProjectPage() {
  ...
  useEffect(() => {
    ...
    async function loadProject() {
      ...
      try {
        ...
        if (active)
          setError(
            apiError?.response?.data?.message || "Could not load project.",
          );
      } finally {
  ...
  useEffect(() => {
    ...
    async function loadPayments() {
      ...
      try {
        ...
        const projectPayments = result.filter(
  ...
  const dynamicJourneySteps = useMemo(
    () => buildJourneySteps(project),
    [project],
  );
  const dynamicRecentActivity = useMemo(
    () => buildRecentActivity(project),
    [project],
  );
  ...
  const pendingAmount = payments
    ...
    .reduce((sum, payment) => sum + payment.amount, 0);
  const activeStep =
    dynamicJourneySteps.find((step) => step.active) ?? dynamicJourneySteps[0];
  const completedSteps = dynamicJourneySteps.filter((step) => step.done).length;
  const progressPercent = Math.max(
    12,
    Math.round((completedSteps / dynamicJourneySteps.length) * 100),
  );
  const dynamicProjectMeta = useMemo(
    () => [
      ...
      },
      {
        label: "System Size",
        value: project
          ? `${project.system.sizeKw}kW ${project.system.panelType}`
          : "-",
        icon: <SolarPowerRoundedIcon sx={{ fontSize: "0.95rem" }} />,
      },
      {
          ...
          <Stack spacing={{ xs: 3.2, md: 3.8 }}>
            {project?.status === "cancelled" && (
              ...
              >
                ...
                <Box>
                  ...
                  <Typography
                    sx={{ color: "#B71C1C", fontSize: "0.78rem", mt: 0.3 }}
                  >
                    The assigned vendor was unable to complete the site visit.
                    Our team is working on reassigning your project to a new
                    vendor. You will be notified once a new vendor is assigned.
                  </Typography>
                </Box>
              </Box>
            )}
            ...
            >
              ...
              >
                <Button
                  ...
                  to={
                    project
                      ? `/service-support/request?projectId=${project.id}`
            ...
            {!isLoading && !error && !project ? (
              <Alert severity="info" sx={{ borderRadius: "0.9rem" }}>
                No installation project is active yet. Please select a vendor
                quote first.
              </Alert>
            ) : null}
                      ...
                      <Stack spacing={2}>
                        <Stack direction="row" spacing={1} alignItems="center">
                          ...
                          >
                            ...
                            {Math.min(
                              completedSteps + 1,
                              dynamicJourneySteps.length,
                            )}{" "}
                            of {dynamicJourneySteps.length}
                          </Box>
                        </Stack>
                                  ...
                                  <Box
                                    sx={{
                                      ...
                                      boxShadow: step.active
                                        ? "0 6px 14px rgba(14,86,200,0.12)"
                                        : "none",
                                      display: "grid",
                                      placeItems: "center",
                                      zIndex: 1,
                      ...
                      <Stack spacing={1.25}>
                        ...
                        >
                          ...
                          <PaymentsOutlinedIcon
                            sx={{ fontSize: "1.05rem", color: "#0E56C8" }}
                          />
                        </Stack>
