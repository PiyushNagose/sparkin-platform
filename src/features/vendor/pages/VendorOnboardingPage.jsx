...
function getCompletion(company, documents) {
  const checks = [
    ...
    documents.some((document) => document.type === "company")
      ? "company-doc"
      : "",
    documents.some((document) => document.type === "certification")
      ? "certification"
      : "",
  ];
...
function getMissingRequiredFields(company, documents) {
  ...
  if (!documents.some((document) => document.type === "company"))
    missing.push("Electrical license");
  if (!documents.some((document) => document.type === "certification"))
    missing.push("Liability insurance");
...
function OnboardingField({
  ...
  return (
    <Box>
      <Typography
        sx={{ mb: 0.5, color: "#3B4658", fontSize: "0.76rem", fontWeight: 700 }}
      >
        {label}
      </Typography>
      <TextField
...
export default function VendorOnboardingPage() {
        ...
        if (active)
          setError(
            apiError?.response?.data?.message ||
              "Could not load onboarding profile.",
          );
      } finally {
  ...
  const completion = useMemo(
    () => getCompletion(company, documents),
    [company, documents],
  );
  const tierCards = [
    {
  ...
  async function uploadDocument(type, event) {
    ...
    try {
      ...
      const profile = await vendorsApi.uploadDocument({
        type,
        title:
          type === "company" ? "Electrical License" : "Liability Insurance",
        fileName: file.name,
        mimeType: file.type,
        data,
      });
  ...
  async function submitApplication() {
    ...
    try {
      ...
      window.dispatchEvent(
        new CustomEvent("sparkin:vendor-profile-updated", {
          detail: submittedProfile,
        }),
      );
      navigate("/vendor/pending-approval", { replace: true });
    } catch (apiError) {
      setError(
        apiError?.response?.data?.message || "Could not submit application.",
      );
    } finally {
      setIsSaving(false);
    }
        ...
        >
          <Box>
            ...
            <Typography
              sx={{
                mt: 0.35,
                color: uploaded ? "#108A49" : "#7A879A",
                fontSize: "0.74rem",
                fontWeight: uploaded ? 700 : 400,
              }}
            >
  ...
  return (
    ...
    >
      <Box
        component="main"
        sx={{ flex: 1, px: { xs: 2, md: 4 }, py: { xs: 3.2, md: 4.2 } }}
      >
        <Box sx={{ maxWidth: 1120, mx: "auto" }}>
          <Box sx={{ textAlign: "center", mb: { xs: 3.5, md: 4.25 } }}>
            ...
            >
              Finalize your professional profile to start receiving project
              leads.
            </Typography>
          </Box>
              ...
              <Grid container spacing={{ xs: 1.8, md: 2 }}>
                ...
                <Grid size={{ xs: 12, md: 6 }}>
                  <OnboardingField
                    ...
                    onChange={(value) => updateCompany("businessType", value)}
              ...
              <Grid container spacing={{ xs: 2, md: 2.4 }} alignItems="center">
                <Grid size={{ xs: 12, md: 5 }}>
                  ...
                  <Stack direction="row" spacing={1.2}>
                    {[
                      ...
                      const isSelected =
                        Number(company.experienceYears) === option.value;
                      ...
                      return (
                        <Button
                          ...
                          onClick={() =>
                            updateCompany("experienceYears", option.value)
                          }
                          sx={{
                            minWidth: 92,
                            minHeight: 42,
                            ...
                            textTransform: "none",
                            "&:hover": {
                              bgcolor: isSelected ? "#EEF3FF" : "#E9EDF3",
                              boxShadow: "none",
                            },
                          }}
                        >
                ...
                <Grid size={{ xs: 12, md: 7 }}>
                  ...
                  <Slider
                    ...
                    step={1}
                    onChange={(_, value) =>
                      updateCompany("projectsCompleted", value)
                    }
                    sx={{
                      color: "#0E56C8",
                ...
                <Grid size={{ xs: 12, md: 4 }}>
                  <OnboardingField
                    ...
                    value={company.totalCapacityMw}
                    type="number"
                    onChange={(value) =>
                      updateCompany("totalCapacityMw", value)
                    }
                  />
                </Grid>
              </Grid>
...
function StepTitle({ number, title }) {
  return (
    <Stack direction="row" spacing={1} alignItems="center" sx={{ mb: 2 }}>
      ...
      <Typography
        sx={{ color: "#18253A", fontSize: "1.32rem", fontWeight: 800 }}
      >
        {title}
      </Typography>
    </Stack>
