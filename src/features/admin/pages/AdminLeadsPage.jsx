  ...
  return (
    <Dialog open={open} onClose={saving ? undefined : onClose} maxWidth="md" fullWidth>
      <Box component="form" onSubmit={handleSubmit}>
        <DialogTitle sx={{ color: adminUi.colors.text, fontWeight: 900 }}>Create New Lead</DialogTitle>
        <DialogContent dividers sx={{ borderColor: adminUi.colors.border }}>
          {error ? <AdminErrorState>{error}</AdminErrorState> : null}
          <Grid container spacing={1.6}>
            <Grid item xs={12} md={4}>
              <TextField required fullWidth label="Customer Name" value={form.fullName} onChange={(e) => updateField("fullName", e.target.value)} />
            </Grid>
            <Grid item xs={12} md={4}>
              <TextField required fullWidth label="Phone Number" value={form.phoneNumber} onChange={(e) => updateField("phoneNumber", e.target.value)} />
            </Grid>
            <Grid item xs={12} md={4}>
              <TextField fullWidth type="email" label="Email" value={form.email} onChange={(e) => updateField("email", e.target.value)} />
            </Grid>
            <Grid item xs={12}>
              <TextField required fullWidth label="Street Address" value={form.street} onChange={(e) => updateField("street", e.target.value)} />
            </Grid>
            <Grid item xs={12} md={4}>
              <TextField required fullWidth label="City" value={form.city} onChange={(e) => updateField("city", e.target.value)} />
            </Grid>
            <Grid item xs={12} md={4}>
              <TextField required fullWidth label="State" value={form.state} onChange={(e) => updateField("state", e.target.value)} />
            </Grid>
            <Grid item xs={12} md={4}>
              <TextField required fullWidth label="Pincode" value={form.pincode} onChange={(e) => updateField("pincode", e.target.value)} />
            </Grid>
            <Grid item xs={12} md={4}>
              <TextField fullWidth type="number" label="System Size (kW)" value={form.systemSizeKw} onChange={(e) => updateField("systemSizeKw", e.target.value)} />
            </Grid>
            <Grid item xs={12} md={4}>
              <FormControl fullWidth>
                <InputLabel>Property Type</InputLabel>
                <Select label="Property Type" value={form.propertyType} onChange={(e) => updateField("propertyType", e.target.value)}>
            ...
            <Grid item xs={12} md={4}>
              <FormControl fullWidth>
                ...
                <Select label="Roof Size" value={form.roofSize} onChange={(e) => updateField("roofSize", e.target.value)}>
            ...
            <Grid item xs={12}>
              <TextField fullWidth multiline minRows={3} label="Notes" value={form.notes} onChange={(e) => updateField("notes", e.target.value)} />
            </Grid>
          </Grid>
        </DialogContent>
        <DialogActions sx={{ px: 3, py: 2 }}>
          <Button onClick={onClose} disabled={saving} sx={{ textTransform: "none", fontWeight: 800 }}>
            Cancel
          </Button>
...
export default function AdminLeadsPage() {
  ...
  async function loadLeads() {
    ...
    try {
      ...
      setState({
        loading: false,
        error: error?.response?.data?.message || error.message || "Unable to load leads",
        data: null,
      });
    }
  }
  ...
  const rows = useMemo(() => {
    ...
    return leads.map((lead) => {
      ...
      const vendorNames = leadQuotes
        .map((quote) => vendorById.get(quote.vendorId)?.company?.name || quote.vendorEmail || quote.vendorId)
        .filter(Boolean);
      ...
      return {
        ...
        statusMeta: meta,
        paymentStatus,
        paymentLabel: paymentStatus === "paid" ? "Paid" : paymentStatus === "pending" ? "Payment Pending" : "No Payment",
  ...
  const locations = useMemo(
    () => [...new Set(rows.map((row) => row.raw.installationAddress?.state).filter(Boolean))],
    [rows],
  );
  ...
  const filteredRows = useMemo(() => {
    ...
    return rows.filter((row) => {
      ...
      const matchesLocation = filters.location === "all" || lead.installationAddress?.state === filters.location;
      const matchesPayment = filters.payment === "all" || row.paymentStatus === filters.payment;
      const createdAt = new Date(lead.createdAt || lead.submittedAt).getTime();
      const matchesDate =
        filters.dateRange === "all" ||
        (!Number.isNaN(createdAt) && now - createdAt <= Number(filters.dateRange) * 86400000);
      const matchesQuery =
        !q ||
      ...
      return matchesStatus && matchesLocation && matchesPayment && matchesDate && matchesQuery;
    });
  }, [rows, filters]);

  const metrics = useMemo(() => {
    const totalCapacity = filteredRows.reduce((sum, row) => sum + Number(row.systemSizeKw || 0), 0);
    const quotes = state.data?.quotes || [];
    const pipeline = quotes.reduce((sum, quote) => sum + Number(quote.pricing?.totalPrice || 0), 0);
    const assigned = rows.filter((row) => row.raw.status === "quote_selected").length;

    return {
      totalCapacity,
      pipeline,
      conversion: rows.length ? Math.round((assigned / rows.length) * 1000) / 10 : 0,
    };
  ...
  async function handleCreateLead(payload) {
    ...
    try {
      ...
      setFormError(error?.response?.data?.message || error.message || "Could not create lead");
    } finally {
      setSaving(false);
    }
  }
  ...
  function handleExport() {
    downloadCsv(`sparkin-admin-leads-${new Date().toISOString().slice(0, 10)}.csv`, buildCsv(filteredRows));
  }
  ...
  return (
    <AdminPageShell>
      <AdminPageHeader
        ...
        actions={
          <>
            ...
            <AdminPrimaryButton startIcon={<AddRoundedIcon />} onClick={() => setDialogOpen(true)} sx={{ borderRadius: "999px", minHeight: 40 }}>
              Create New Lead
            </AdminPrimaryButton>
          </>
        }
              ...
              InputProps={{
                ...
                endAdornment: (
                  <InputAdornment position="end">
                    <FilterListRoundedIcon sx={{ color: "#0E56C8", fontSize: "1rem" }} />
                  </InputAdornment>
                ),
              }}
            />
          ...
          <Table sx={{ minWidth: 980 }}>
            <TableHead>
              <TableRow sx={{ bgcolor: "#F6F8FB" }}>
                {["Lead ID", "Customer Name", "Location", "Size (kW)", "Status", "Payment", "Vendors", "Actions"].map((heading) => (
                  <TableCell key={heading} sx={{ color: "#738096", fontSize: "0.66rem", fontWeight: 900, letterSpacing: "0.11em", textTransform: "uppercase", py: 1.7 }}>
                    {heading}
                  </TableCell>
                ))}
              </TableRow>
              ...
              {filteredRows.length ? (
                filteredRows.map((row) => (
                  <TableRow key={row.raw.id} hover sx={{ "& td": { borderColor: "#EEF2F6", py: 2 } }}>
                    ...
                    <TableCell>
                      <LeadPill color={row.statusMeta.color} bg={row.statusMeta.bg}>{row.statusLabel}</LeadPill>
                    </TableCell>
                    <TableCell>
                      <Box
                        sx={{
                ...
                <TableRow>
                  <TableCell colSpan={8}>
                    <AdminEmptyState title="No leads found" subtitle="Adjust filters or create a new operational lead." />
                  </TableCell>
                </TableRow>
              )}
            </TableBody>
      ...
      <Grid container spacing={2.2} sx={{ mt: 2.8 }}>
        <Grid item xs={12} md={4}>
          <MetricCard icon={BoltOutlinedIcon} title="Total Capacity" value={`${metrics.totalCapacity.toFixed(1)} kW`} accent="#0E56C8" />
        </Grid>
        <Grid item xs={12} md={4}>
          <MetricCard icon={TrendingUpRoundedIcon} title="Revenue Pipeline" value={`₹${(metrics.pipeline / 100000 || 0).toFixed(1)}L`} accent="#8A9700" />
        </Grid>
        <Grid item xs={12} md={4}>
          <MetricCard icon={GroupAddOutlinedIcon} title="Conversion Rate" value={`${metrics.conversion}%`} accent="#10985E" />
        </Grid>
      </Grid>
