...
function QuickPreview({ estimate, onGetFullReport, onProceedToBooking }) {
  const roofUtilizationPercent =
    estimate.system.roofUtilizationPercent ||
    Math.round(
      clamp(
        (Number(estimate.system.requiredRoofAreaSqFt || 0) /
          Number(estimate.input.roofAreaSqFt || estimate.system.requiredRoofAreaSqFt || 1)) *
          100,
        1,
        100,
      ),
  ...
  return (
    <Stack spacing={{ xs: 2.6, md: 3.15 }}>
      ...
      <Stack direction={{ xs: "column", md: "row" }} justifyContent="space-between" spacing={2.2}>
        <Box>
          <Box sx={{ display: "inline-flex", px: 1.1, py: 0.35, borderRadius: 999, bgcolor: "#FFF8E6", border: "1px solid #F0C419", mb: 1.2 }}>
            <Typography sx={{ color: "#7A6B00", fontSize: "0.68rem", fontWeight: 800, letterSpacing: "0.06em", textTransform: "uppercase" }}>
              Quick Estimate · Based on bill & roof area
            </Typography>
          </Box>
          ...
          <Typography sx={{ mt: 0.8, maxWidth: 560, color: "#697790", ...publicTypography.body }}>
            Estimate based on a monthly bill of {formatMoney(estimate.input.monthlyBill)} and {formatNumber(estimate.input.roofAreaSqFt)} sq. ft. of available roof area.
          </Typography>
        </Box>
        <Stack direction={{ xs: "row", md: "column" }} spacing={1} sx={{ alignSelf: { md: "center" } }}>
          <Button
          ...
          <Button
            ...
            variant="outlined"
            sx={{ minHeight: 44, px: 2.2, borderRadius: "0.9rem", borderColor: "#0E56C8", color: "#0E56C8", fontWeight: 800, textTransform: "none" }}
          >
            Full Report
          </Button>
        </Stack>
      ...
      {/* CTA banner */}
      <Box sx={{ p: { xs: 2.2, md: 2.8 }, borderRadius: "1.2rem", background: "linear-gradient(135deg, #0E56C8 0%, #0A3E9A 100%)", color: "white" }}>
        <Stack direction={{ xs: "column", md: "row" }} justifyContent="space-between" alignItems={{ xs: "flex-start", md: "center" }} spacing={2}>
          <Box>
            <Typography sx={{ fontWeight: 800, fontSize: { xs: "1.1rem", md: "1.3rem" } }}>
              Want a more accurate estimate?
            </Typography>
            <Typography sx={{ mt: 0.5, color: "rgba(255,255,255,0.8)", fontSize: "0.88rem", lineHeight: 1.6 }}>
              Add your pincode, city, connection type and roof details for a full state-specific report with subsidy breakdown and EMI options.
            </Typography>
          </Box>
          <Button
            onClick={onGetFullReport}
            variant="contained"
            endIcon={<ArrowForwardRoundedIcon />}
            sx={{ flexShrink: 0, minHeight: 48, px: 2.8, borderRadius: "0.9rem", bgcolor: "white", color: "#0E56C8", fontWeight: 800, textTransform: "none", "&:hover": { bgcolor: "#F0F5FF" } }}
          >
            Get Full Report
          </Button>
        </Stack>
      ...
      <Box sx={{ p: 1.6, borderRadius: "1rem", bgcolor: "#F3F7FC", border: "1px solid rgba(220,228,239,0.96)" }}>
        <Stack direction="row" spacing={1.1} alignItems="flex-start">
          ...
          <Typography sx={{ color: "#627084", fontSize: "0.78rem", lineHeight: 1.65 }}>
            Quick estimate based on monthly bill and roof area only. For a precise report with subsidy, EMI, and investment breakdown, use the full calculator with your pincode and city.
          </Typography>
        </Stack>
      </Box>
    </Stack>
...
// Full result shown when coming from the calculator page (all inputs provided)
function FullResult({ estimate, onProceedToBooking }) {
        ...
        <Grid size={{ xs: 12, md: 6 }}>
          <Box sx={{ minHeight: "100%", borderRadius: "1rem", overflow: "hidden", backgroundImage: `linear-gradient(180deg, rgba(5,16,28,0.05) 0%, rgba(8,17,28,0.72) 86%), url(${resultBannerImage})`, backgroundSize: "cover", backgroundPosition: "center", display: "flex", alignItems: "flex-end" }}>
            <Box sx={{ p: { xs: 2.2, md: 2.55 }, color: "white", maxWidth: 420 }}>
              ...
              <Button onClick={onProceedToBooking} endIcon={<ArrowForwardRoundedIcon />} sx={{ mt: 1.2, color: "white", px: 0, fontWeight: 800, textTransform: "none" }}>
                Proceed to booking
              </Button>
            </Box>
          </Box>
...
export default function CalculatorResultsPage() {
  ...
  const { applyCalculatorEstimate } = useBookingDraft();
  const [estimate, setEstimate] = useState(null);
  const [isQuickMode, setIsQuickMode] = useState(false);
  const [loading, setLoading] = useState(true);

  useEffect(() => {
    const stored = calculatorStorage.getEstimate();

    // Case 1: came from full calculator — use stored estimate
    if (stored) {
      setEstimate(stored);
      setIsQuickMode(stored.input?.source === "home_quick_tool");
      setLoading(false);
      return;
    }

    // Case 2: no backend estimate available — send to calculator
  ...
  function handleProceedToBooking() {
    applyCalculatorEstimate(estimate);
    navigate("/booking");
  }

  if (loading) {
    return (
      <Box className={styles.pageShell} sx={{ display: "flex", alignItems: "center", justifyContent: "center", minHeight: "100vh" }}>
        <Stack alignItems="center" spacing={2}>
          <CircularProgress size={36} sx={{ color: "#0E56C8" }} />
          <Typography sx={{ color: "#697790", fontSize: "0.9rem" }}>Calculating your solar potential...</Typography>
        </Stack>
      </Box>
    );
  }
    ...
    <Box className={styles.pageShell}>
      ...
      >
        <Container maxWidth={false} disableGutters className={styles.compactContainer}>
          {isQuickMode
            ? <QuickPreview estimate={estimate} onGetFullReport={handleGetFullReport} onProceedToBooking={handleProceedToBooking} />
            : <FullResult estimate={estimate} onProceedToBooking={handleProceedToBooking} />
          }
        </Container>
      </Box>
    </Box>
