...
function getRating(vendor) {
  ...
  return Math.min(
    5,
    Math.max(3.1, 3.5 + docs * 0.22 + Math.min(experience, 12) * 0.07),
  );
}
  ...
  return vendors
    ...
    .map((vendor) => {
      const assignedLeads = leads.filter((lead) =>
        lead.assignedVendorIds?.includes(vendor.vendorId),
      );
      const vendorQuotes = quotes.filter(
        (quote) => quote.vendorId === vendor.vendorId,
      );
      const rejectedQuotes = vendorQuotes.filter(
...
function NetworkCard({ activeCount, capacityMw, growth }) {
  return (
    ...
    >
      ...
      <Typography
        sx={{ mt: 0.8, color: "#CFE0FF", fontSize: "0.82rem", lineHeight: 1.6 }}
      >
        Vendor onboarding has changed by {growth}% this quarter, based on
        verified network movement.
      </Typography>
      <Grid container spacing={2.2} sx={{ mt: 2.6 }}>
        <Grid size={{ xs: 6 }}>
          ...
          <Typography sx={{ mt: 0.4, fontSize: "1.75rem", fontWeight: 950 }}>
            {activeCount}
          </Typography>
        </Grid>
        <Grid size={{ xs: 6 }}>
...
export default function AdminVendorsPage() {
  ...
  const regions = useMemo(
    () => [
      ...new Set(
        rows
          .flatMap((row) => [
            row.vendor.company?.city,
            row.vendor.company?.state,
            row.vendor.company?.coverageArea,
          ])
          .filter(Boolean),
      ),
    ],
  ...
  const filteredRows = useMemo(() => {
    ...
    return rows.filter((row) => {
      ...
      const searchable = [
        ...
        .toLowerCase();
      const statusMatch =
        filters.status === "all" || row.status.key === filters.status;
      const regionMatch =
        filters.region === "all" ||
        location.includes(filters.region.toLowerCase());
      const ratingMatch =
        filters.rating === "all" || row.rating >= Number(filters.rating);
  ...
  const metrics = useMemo(() => {
    ...
    const submittedRows = rows.filter(
      (row) => row.vendor.verificationStatus === "submitted",
    );
    const growth = rows.length
      ? Math.round((submittedRows.length / rows.length) * 100)
      : 0;
  ...
  async function updateStatus(verificationStatus) {
    ...
    setActionError("");
    try {
      await adminVendorsApi.updateVendorStatus(activeVendor.vendorId, {
        verificationStatus,
      });
      closeMenu();
      await loadVendors();
    ...
    <AdminPageShell>
      ...
      <AdminPanel sx={{ overflow: "hidden" }}>
                ...
                filteredRows.map((row) => {
                  ...
                  return (
                    <TableRow
                      key={vendor.vendorId}
                      hover
                      sx={{ "& td": { borderColor: "#EEF2F6", py: 1.8 } }}
                    >
                      <TableCell>
                        <Stack
                          direction="row"
                      ...
                      <TableCell>
                        ...
                        >
                          <StarRoundedIcon
                            sx={{ color: "#7B8500", fontSize: "0.95rem" }}
                          />
                          <Typography
                            sx={{
                              color: adminUi.colors.text,
                      ...
                      >
                        {row.leadsCount.toLocaleString("en-IN")}
                      </TableCell>
                      <TableCell
                        sx={{
                          color: adminUi.colors.text,
                      ...
                      >
                        {row.participationCount.toLocaleString("en-IN")}
                      </TableCell>
                      <TableCell
                        sx={{
                          color: "#D94444",
                      ...
                      >
                        {row.rejectedCount.toLocaleString("en-IN")}
                      </TableCell>
                      <TableCell>
                ...
                <TableRow>
                  <TableCell colSpan={9}>
                    <AdminEmptyState
                      ...
                      subtitle="Adjust filters or ask vendors to complete onboarding."
                    />
                  </TableCell>
                </TableRow>
              )}
        ...
        <Stack
          ...
          spacing={1.5}
          sx={{ px: 2, py: 1.6, borderTop: "1px solid #EEF2F6" }}
        >
          <Typography
