...
export const referralsController = {
  ...
  async trackSignup(req, res) {
    const result = await referralsService.trackSignup(req.auth, req.body);
    res.status(200).json(result);
  },

  async trackBooking(req, res) {
    const result = await referralsService.trackBooking(req.auth, req.body);
    res.status(200).json(result);
  },
  ...
  async getSettings(req, res) {
    const settings = await referralsService.getAdminReferralSettings(req.auth);
    res.status(200).json({ settings });
  },

  async updateSettings(req, res) {
    const settings = await referralsService.updateAdminReferralSettings(
      req.auth,
      req.body,
    );
    res.status(200).json({ settings });
